...
func (s *ProcessingTestSuite) SetupSuite() {
	...
	s.fetcher = testutil.NewDefaultFetcher(s.T())
	s.idf = imagedata.NewFactory(s.fetcher)
...
func (s *ProcessingTestSuite) TestResizeToFit() {
	...
	for _, tc := range testCases {
		s.Run(fmt.Sprintf("%dx%d", tc.width, tc.height), func() {
			...
			result, err := ProcessImage(context.Background(), imgdata, po, s.idf, s.fetcher)
			s.Require().NoError(err)
...
func (s *ProcessingTestSuite) TestImageResolutionTooLarge() {
	...
	imgdata := s.openFile("test2.jpg")
	_, err := ProcessImage(context.Background(), imgdata, po, s.idf, s.fetcher)
