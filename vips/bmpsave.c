...
typedef struct _VipsForeignSaveBmpBuffer {
  VipsForeignSaveBmp parent_object;

  VipsArea *buf;
} VipsForeignSaveBmpBuffer;

typedef VipsForeignSaveBmpClass VipsForeignSaveBmpBufferClass;

G_DEFINE_TYPE(VipsForeignSaveBmpBuffer, vips_foreign_save_bmp_buffer,
    vips_foreign_save_bmp_get_type());

static int
vips_foreign_save_bmp_buffer_build(VipsObject *object)
{
  VipsForeignSaveBmp *bmp = (VipsForeignSaveBmp *) object;
  VipsForeignSaveBmpBuffer *buffer = (VipsForeignSaveBmpBuffer *) object;

  VipsBlob *blob;

  if (!(bmp->target = vips_target_new_to_memory()))
    return -1;

  if (VIPS_OBJECT_CLASS(vips_foreign_save_bmp_buffer_parent_class)
          ->build(object))
    return -1;

  g_object_get(bmp->target, "blob", &blob, NULL);
...
static void
vips_foreign_save_bmp_buffer_class_init(VipsForeignSaveBmpBufferClass *class)
{
  GObjectClass *gobject_class = G_OBJECT_CLASS(class);
  VipsObjectClass *object_class = (VipsObjectClass *) class;
  ...
  object_class->nickname = "bmpsave_buffer";
  object_class->description = "save image to bmp buffer";
  object_class->build = vips_foreign_save_bmp_buffer_build;

  VIPS_ARG_BOXED(class, "buffer", 1,
      "Buffer",
      "Buffer to save to",
      VIPS_ARGUMENT_REQUIRED_OUTPUT,
      G_STRUCT_OFFSET(VipsForeignSaveBmpBuffer, buf),
      VIPS_TYPE_BLOB);
}

static void
vips_foreign_save_bmp_buffer_init(VipsForeignSaveBmpBuffer *buffer)
{
}

/**
 * vips_bmpsave_buffer: (method)
 * @in: image to save
 * @buf: (array length=len) (element-type guint8): return output buffer here
 * @len: (type gsize): return output length here
 * @...: `NULL`-terminated list of optional named arguments
 *
 * As [method@Image.bmpsave], but save to a memory buffer.
 *
 ...
 *
 * ::: seealso
 *     [method@Image.bmpsave], [method@Image.write_to_file].
 *
 * Returns: 0 on success, -1 on error.
 */
int
vips_bmpsave_buffer(VipsImage *in, void **buf, size_t *len, ...)
{
  va_list ap;
  VipsArea *area;
  int result;
  ...
  va_start(ap, len);
  result = vips_call_split("bmpsave_buffer", ap, in, &area);
  va_end(ap);
