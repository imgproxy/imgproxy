...
* `IMGPROXY_FALLBACK_TTL`: a duration (in seconds) sent via the `Expires` and `Cache-Control: max-age` HTTP headers when fallback response. This feature is disabled by default.
