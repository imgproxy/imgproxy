...
func New(ctx context.Context, config *Config) (*ImgProxy, error) {
	...
	fallbackImage, err := auximageprovider.NewStaticProvider(ctx, idf, config.FallbackImage, "fallback image")
	if err != nil {
		return nil, err
	}
	...
	ph, err := processinghandler.New(
		streamHandler, headerWriter, semaphores, fallbackImage, idf, config.ProcessingHandler, fetcher,
	)
	if err != nil {
		return nil, err
	}
