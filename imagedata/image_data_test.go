...
func (s *ImageDataTestSuite) TestDownloadStatusPartialContent() {
	...
	for _, tc := range testCases {
		s.Run(tc.name, func() {
			...
			if tc.expectErr {
				s.Require().Error(err)
				s.Require().Equal(http.StatusNotFound, errctx.Wrap(err, 0).StatusCode())
			} else {
				s.Require().NoError(err)
				s.Require().NotNil(imgdata)
				s.Require().True(testutil.ReadersEqual(s.T(), bytes.NewReader(s.data), imgdata.Reader()))
...
func (s *ImageDataTestSuite) TestDownloadStatusNotFound() {
	...
	s.Require().Error(err)
	s.Require().Equal(404, errctx.Wrap(err, 0).StatusCode())
...
func (s *ImageDataTestSuite) TestDownloadStatusForbidden() {
	...
	s.Require().Error(err)
	s.Require().Equal(403, errctx.Wrap(err, 0).StatusCode())
	s.Require().Nil(imgdata)
}
...
func (s *ImageDataTestSuite) TestDownloadStatusInternalServerError() {
	...
	s.Require().Error(err)
	s.Require().Equal(502, errctx.Wrap(err, 0).StatusCode())
	s.Require().Nil(imgdata)
}
...
func (s *ImageDataTestSuite) TestDownloadUnreachable() {
	...
	s.Require().Error(err)
	s.Require().Equal(500, errctx.Wrap(err, 0).StatusCode())
	s.Require().Nil(imgdata)
}
...
func (s *ImageDataTestSuite) TestDownloadInvalidImage() {
	...
	s.Require().Error(err)
	s.Require().Equal(http.StatusUnprocessableEntity, errctx.Wrap(err, 0).StatusCode())
	s.Require().Nil(imgdata)
}
