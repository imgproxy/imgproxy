...
// and byte slice.
func (f *Factory) NewFromBytesWithFormat(format imagetype.Type, b []byte) ImageData {
...
func (f *Factory) NewFromBytes(b []byte) (ImageData, error) {
	...
	return f.NewFromBytesWithFormat(format, b), nil
}
...
func (f *Factory) DownloadSync(ctx context.Context, imageURL, desc string, opts DownloadOptions) (ImageData, http.Header, error) {
	...
	d := f.NewFromBytesWithFormat(format, b)
	return d, h, nil
}
