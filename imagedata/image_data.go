...
func Init() error {
	...
	// if err := loadWatermark(Fetcher); err != nil {
	// 	return err
	// }
...
// func loadWatermark(fetcher *fetcher.Fetcher) error {
// 	var err error

// 	f := NewFactory(fetcher)

// 	switch {
// 	case len(config.WatermarkData) > 0:
// 		Watermark, err = f.NewFromBase64(config.WatermarkData)
...
// 		Watermark, err = f.NewFromPath(config.WatermarkPath)
// 		if err != nil {
// 			return ierrors.Wrap(err, 0, ierrors.WithPrefix("can't read watermark from file"))
// 		}

// 	case len(config.WatermarkURL) > 0:
// 		Watermark, _, err = f.DownloadSync(context.Background(), config.WatermarkURL, "watermark", DefaultDownloadOptions())
// 		if err != nil {
